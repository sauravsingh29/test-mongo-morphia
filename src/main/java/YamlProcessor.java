...
/**
 * YAML parsing and loading to Properties.
 *
...
public class YamlProcessor {
    ...
    private final ResolutionMethod resolutionMethod = ResolutionMethod.OVERRIDE;

    private final List<DocumentMatcher> documentMatchers = Collections.emptyList();

    private final boolean matchDefault = true;


    protected void process(MatchCallback callback, InputStream resource) {
    ...
    private void buildFlattenedMap(Map<String, Object> result, Map<String, Object> source, String path) {
        source.forEach((key, value) -> {
            if (path != null && !path.isEmpty() && path.trim().length() > 0) {
                if (key.startsWith("[")) {
                    key = path + key;
                } else {
                    key = path + '.' + key;
